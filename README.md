...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/nass_query \
  -H 'Content-Type: application/json' \
  -d '{"commodity":"CORN","stat_category":"YIELD","state":"IOWA","year":"2024"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/nass_query`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
