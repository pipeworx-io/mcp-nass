...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'NASS');
}
...
// QuickStats answers an outage with a full HTML page — stylesheet, inline
// JavaScript, and the actual message ("Please come back later") repeated in
// twelve languages near the bottom. Passing that through meant an agent asking
// about corn prices got ~3KB of markup and no idea what had happened. Pull the
// human sentence out when we can find it, and never hand back raw markup.
function summarizeUpstreamError(body: string): string {
  const text = body.trim();
  if (!/^\s*<(?:!doctype|html|\?xml)/i.test(text)) return text.slice(0, 300);
  // The multilingual block is "(en) Please come back later\n(fr) ...". Take the
  // English line if present; otherwise fall back to the <title>.
  const en = text.match(/\(en\)\s*([^\n<]+)/i)?.[1]?.trim();
  const title = text.match(/<title>([^<]+)<\/title>/i)?.[1]?.trim();
  const detail = en ?? title;
  return detail
    ? `upstream returned an HTML error page — "${detail}". This is a USDA QuickStats outage, not a problem with the query; retry shortly.`
    : 'upstream returned an HTML error page instead of JSON, which usually means a USDA QuickStats outage. Retry shortly.';
}
...
async function nassGet(apiKey: string, params: Record<string, string>): Promise<unknown> {
  ...
  if (!res.ok) {
    throw new Error(`NASS API error (${res.status}): ${summarizeUpstreamError(await res.text())}`);
  }
...
// Month abbreviations used in NASS reference_period_desc (e.g. "JUN 2025")
...
// Convert a NASS year + reference_period_desc to an ISO date string.
// Weekly crop progress: "WEEK #28" → Sunday ending that week.
// Monthly prices:       "JUN 2025" → "2025-06-01".
// Annual/census:        "YEAR - ANN" → "YYYY-01-01".
function parsePeriodDate(year: number | undefined, period: string | undefined): string | null {
  if (!year || !period) return null;
  const p = String(period).trim().toUpperCase();

  // Weekly: "WEEK #28" — compute the Sunday that ends that USDA week
  const wm = p.match(/WEEK\s*#\s*(\d+)/);
  if (wm) {
    ...
    const firstSunMs = Date.UTC(year, 0, 1 + daysToFirstSun);
    const weekEndMs = firstSunMs + (weekNum - 1) * 7 * 86400000;
    return new Date(weekEndMs).toISOString().slice(0, 10);
  }

  // Monthly: "JUN 2025", "JAN 2024", etc.
  const mon = p.slice(0, 3);
  if (MONTH_MAP[mon]) return `${year}-${MONTH_MAP[mon]}-01`;

  // Annual / census: "YEAR - ANN", "ANNUAL", etc.
  if (p.includes('YEAR') || p.includes('ANNUAL') || p.includes('ANN')) return `${year}-01-01`;
...
function formatRecords(data: { data?: NassRecord[] }): unknown {
  ...
  // Derive latest_date from the most recent year+period so callers (and golden
  // recentWithin checks) can find an ISO date without scanning all records.
  let latestDate: string | null = null;
  if (records.length > 0) {
    const maxYear = Math.max(...records.map((r) => r.year ?? 0).filter(Boolean));
    if (maxYear > 0) {
      const yearRecords = records.filter((r) => r.year === maxYear);
      for (const r of yearRecords) {
        const d = parsePeriodDate(r.year, r.reference_period_desc);
        if (d && (!latestDate || d > latestDate)) latestDate = d;
      }
      if (!latestDate) latestDate = `${maxYear}-01-01`;
    }
  }

  return {
    count: records.length,
    ...(latestDate ? { latest_date: latestDate } : {}),
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        year: { type: 'string', description: 'Year, e.g., "2026". Optional — defaults to current year.' },
        state: { type: 'string', description: 'State name (optional, defaults to national)' },
        stat_category: { type: 'string', description: 'Narrow to "CONDITION" (good/excellent ratings) or "PROGRESS" (planting/harvest %). Optional — omit for both.' },
      },
      required: ['_apiKey', 'commodity'],
    },
  },
...
// Build the crop-condition headline: latest week's national ratings, incl. the
// standard "good OR excellent" combined %. Without this, callers got a 12k-record
// raw dump (truncated), often seeing only 'excellent' rows.
function latestConditionSummary(records: NassRecord[], year: string): Record<string, unknown> | null {
  const cond = records.filter((r) => (r.unit_desc ?? '').toUpperCase().startsWith('PCT ') && (r.agg_level_desc ?? '') === 'NATIONAL');
  if (!cond.length) return null;
  const wk = (p?: string) => {
    const m = String(p ?? '').match(/(\d+)/);
    return m ? Number(m[1]) : -1;
  };
  const latestP = cond.reduce((a, b) => (wk(b.reference_period_desc) > wk(a.reference_period_desc) ? b : a)).reference_period_desc;
  const latest = cond.filter((r) => r.reference_period_desc === latestP);
  const pct = (cat: string) => {
    const rec = latest.find((r) => (r.unit_desc ?? '').toUpperCase() === `PCT ${cat}`);
    return rec && rec.Value != null ? Number(rec.Value) : null;
  };
  const good = pct('GOOD');
  const exc = pct('EXCELLENT');
  if (good == null && exc == null) return null;
  const yearNum = parseInt(year, 10);
  const weekEndDate = parsePeriodDate(Number.isFinite(yearNum) ? yearNum : undefined, latestP);
  return {
    scope: 'NATIONAL',
    year,
    period: latestP ?? null,
    ...(weekEndDate ? { week_ending_date: weekEndDate } : {}),
    very_poor_pct: pct('VERY POOR'),
    poor_pct: pct('POOR'),
    fair_pct: pct('FAIR'),
    good_pct: good,
    excellent_pct: exc,
    good_or_excellent_pct: good != null && exc != null ? good + exc : null,
  };
}

async function nassCropProgress(apiKey: string, args: Record<string, unknown>) {
  const yearStr = (args.year as string) || new Date().getFullYear().toString();
  const params: Record<string, string> = {
    source_desc: 'SURVEY',
    freq_desc: 'WEEKLY',
    commodity_desc: (args.commodity as string).toUpperCase(),
  ...
  if (args.state) params.state_name = (args.state as string).toUpperCase();
  if (!args.state) params.agg_level_desc = 'NATIONAL';
  // Optional narrowing: "CONDITION" (good/excellent ratings) vs "PROGRESS"
  // (planted/emerged/harvested). Without it the API returns both, mixed.
  if (args.stat_category) params.statisticcat_desc = String(args.stat_category).toUpperCase();

  const data = (await nassGet(apiKey, params)) as { data?: NassRecord[] };
  const base = formatRecords(data) as Record<string, unknown>;
  const summary = latestConditionSummary(data.data ?? [], yearStr);
  return summary ? { condition_summary: summary, ...base } : base;
}
